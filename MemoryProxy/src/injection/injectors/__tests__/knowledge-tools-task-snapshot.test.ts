...
const resources: KnowledgeItem[] = [
  {
    ...
    team_id: "team-1",
    user_id: "user-1",
    repo_url: "https://github.com/team-fork/tdai-snapshot-widgets.git",
...
describe("task snapshot code graph anchoring", () => {
  it("uses the Task canonical project only for Task-scoped code graphs", () => {
    const result = applyTaskCanonicalProject(resources, new Set(["cg-snapshot", "wiki-1"]), "acme/widgets");
    expect(result[0]?.repo_slug).toBe("acme/widgets");

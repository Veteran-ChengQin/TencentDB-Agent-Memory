...
check_llm_openai() {
  ...
  local code body_file error_file transport_error=""
  body_file=$(mktemp /tmp/llm-check.XXXXXX)
  error_file="${body_file}.err"
  # DNS/TLS/VPN 切换时偶发的传输错误不应立刻判定配置无效。仅重试不消耗
  # token 的 GET /models；最终仍失败时保留 curl 原始错误用于定位。
  if ! code=$("$CURL" -sS --connect-timeout 5 --max-time 15 \
    --retry 2 --retry-delay 1 --retry-all-errors \
    -o "$body_file" -w "%{http_code}" \
    -H "Authorization: Bearer $key" \
    "$url" 2>"$error_file"); then
    code="000"
    transport_error=$(head -c 180 "$error_file" 2>/dev/null || true)
  fi
  if [[ "$code" == "200" ]]; then
    ...
    check_llm_anthropic "$label" "$base" "$key" "$model" || fallback_status=$?
    rm -f "$body_file" "$error_file"
    return "$fallback_status"
  elif [[ "$code" == "000" ]]; then
    warn "$label 无法连接 ${url}（curl 传输失败）：${transport_error:-未返回错误详情}"
    rm -f "$body_file" "$error_file"
    return 1
  else
...
check_llm_anthropic() {
  ...
  body_file=$(mktemp /tmp/llm-check.XXXXXX)
  error_file="${body_file}.err"
  if ! code=$("$CURL" -sS --connect-timeout 5 --max-time 15 -o "$body_file" -w "%{http_code}" \
  ...
  case "$code" in
    200)
      ...
      rm -f "$body_file" "$error_file"; return 0 ;;
    401|403)
      echo "${C_RED}[error]${C_RST} $label API key 无效（HTTP ${code}）：$url" >&2
      head -c 200 "$body_file" >&2; echo >&2
      rm -f "$body_file" "$error_file"; return 1 ;;
    404)
...
check_llm_from_container() {
  ...
  case "$proto" in
    anthropic)
      ...
      url="$base"
      if ! code=$($DOCKER exec "$container" curl -sS -o /dev/null --max-time 15 \
         -w "%{http_code}" -X POST -H "Content-Type: application/json" \
         -H "x-api-key: $key" -H "anthropic-version: 2023-06-01" \
         -d "{\"model\":\"$model\",\"max_tokens\":1,\"messages\":[{\"role\":\"user\",\"content\":\"ping\"}]}" \
         "$url" 2>/dev/null); then code="000"; fi
      ;;
    *)
      base="${base%/}"; base="${base%/v1}"
      url="${base}/v1/models"
      if ! code=$($DOCKER exec "$container" curl -sS -o /dev/null --max-time 10 \
         -w "%{http_code}" -H "Authorization: Bearer $key" "$url" 2>/dev/null); then code="000"; fi
      ;;
  esac
  if [[ "$code" == "000" ]]; then

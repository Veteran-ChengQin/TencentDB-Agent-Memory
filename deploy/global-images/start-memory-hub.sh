...
$DOCKER run -d --name "$CONTAINER" \
  ...
  -e LLM_MAX_TOKENS="${MEMORY_LLM_MAX_TOKENS:-8192}" \
  -e KNOWLEDGE_LLM_BINDING_SYNC=0 \
  -e TDAI_SNAPSHOT_GITHUB_OWNER="${TDAI_SNAPSHOT_GITHUB_OWNER:-}" \
  -e TDAI_SNAPSHOT_GITHUB_TOKEN="${TDAI_SNAPSHOT_GITHUB_TOKEN:-}" \
  -e TDAI_SNAPSHOT_REPO_PREFIX="${TDAI_SNAPSHOT_REPO_PREFIX:-tdai-snapshot}" \
  -e TDAI_SNAPSHOT_EXCLUDED_PATHS="${TDAI_SNAPSHOT_EXCLUDED_PATHS:-.github/workflows}" \
  -e TDAI_SNAPSHOT_GIT_USER_NAME="${TDAI_SNAPSHOT_GIT_USER_NAME:-TDAI Snapshot Bot}" \
  -e TDAI_SNAPSHOT_GIT_USER_EMAIL="${TDAI_SNAPSHOT_GIT_USER_EMAIL:-tdai-snapshot@localhost}" \

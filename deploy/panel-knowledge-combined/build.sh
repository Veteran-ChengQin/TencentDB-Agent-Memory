...
# Windows checkout may keep shell scripts as CRLF. The runtime image executes
# this file directly, so normalize the copied artifact without rewriting the
# developer's working-tree file.
sed -i 's/\r$//' "$CTX_DIR/start-combined.sh"

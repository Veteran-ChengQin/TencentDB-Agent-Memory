...
import { assembleTaskPatch } from '../../web/src/pages/workbench/WorkbenchPage/components/task-patch.js';
...
describe('task snapshot naming and validation', () => {
  it('derives a canonical GitHub project', () => {
    expect(canonicalProjectFromRepoUrl('https://github.com/mwaskom/seaborn.git')).toBe('mwaskom/seaborn');
    expect(canonicalProjectFromRepoUrl('http://github.com/mwaskom/seaborn.git')).toBeUndefined();
    expect(canonicalProjectFromRepoUrl('https://gitlab.com/mwaskom/seaborn.git')).toBeUndefined();
  });

  it('uses stable repository and branch names', () => {
    expect(snapshotRepositoryName('tdai-snapshot', 'mwaskom/seaborn')).toBe('tdai-snapshot-mwaskom-seaborn');
  ...
  it('assembles mixed collector output with one separator and a final line feed', () => {
    ...
    const second = patch.replaceAll('seaborn/utils.py', 'seaborn/tests/test_utils.py');
  ...
  it('accepts a normal task patch', () => {
    expect(() => validateSnapshotInput({
      taskId: 'task-abc123',
      sourceRepoUrl: 'https://github.com/mwaskom/seaborn.git',
      canonicalProject: 'mwaskom/seaborn',
  ...
  it('rejects secret-bearing files and project mismatches', () => {
    const secretPatch = patch.replaceAll('seaborn/utils.py', '.env');
    expect(() => validateSnapshotInput({
      taskId: 'task-abc123',
      sourceRepoUrl: 'https://github.com/mwaskom/seaborn.git',
      canonicalProject: 'mwaskom/seaborn',
    ...
    expect(() => validateSnapshotInput({
      taskId: 'task-abc123',
      sourceRepoUrl: 'https://github.com/mwaskom/seaborn.git',
      canonicalProject: 'other/seaborn',
      baseCommit: '091f4c0e4f3580a8060de5596fa64c1ff9454dc5',
      patch,
    })).toThrow('不一致');
  });

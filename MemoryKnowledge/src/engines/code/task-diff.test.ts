...
describe("buildTaskCodeGraphDiff", () => {
  it("extracts added files, entities and relations from a unified diff", () => {
    const patch = [
      "diff --git a/pkg/legend.py b/pkg/legend.py",
      "new file mode 100644",
      "--- /dev/null",
      "+++ b/pkg/legend.py",
      "@@ -0,0 +1,5 @@",
      "+from matplotlib import legend",
      "+class LegendMover(BaseMover):",
      "+    pass",
      "+def move_legend(ax):",
      "+    return legend(ax)",
    ].join("\n");
    ...
    expect(result.files).toEqual([{ path: "pkg/legend.py", change_type: "added", additions: 5, deletions: 0 }]);
    expect(result.entities.map((item) => [item.name, item.change_type])).toEqual([
      ["LegendMover", "added"],
      ["move_legend", "added"],
    ]);
    expect(result.relations.some((item) => item.kind === "imports" && item.target === "matplotlib")).toBe(true);
    expect(result.relations.some((item) => item.kind === "inherits" && item.target === "BaseMover")).toBe(true);
    expect(result.relations.some((item) => item.kind === "calls" && item.target === "legend")).toBe(true);
  });
